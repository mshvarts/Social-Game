...
void GameParser::parseGameConfiguration(std::unique_ptr<game::Game> &game, const json &jsonFile)
{
    ...
    game->setMaxNumberOfPlayers(configurationJson[JSON_CONFIG_PLAYER_COUNT][JSON_MAX]);
    game->setMinNumberOfPlayers(configurationJson[JSON_CONFIG_PLAYER_COUNT][JSON_MIN]);
